...
function setGuard(request, response, rules = { required: {}, optional: {} }) {
    ...
    if (missingKeys.length > 0) {
        responseFormat.sendResponseFormat(response, 400, `Missing required attributes`, null, missingKeys.map(key => `\`${key}\` is required`));
        return false;
    }
    ...
    if (allWrongTypes.length > 0) {
        responseFormat.sendResponseFormat( response, 400, `Invalid data type`, null, allWrongTypes.map(({ key, type }) => `\`${key}\` must be of type ${type}`));
        return false;
    }
...
function checkMissingAttributes(body, rules = { required: {}, optional: {} }) {
    const unknownKeys = Object.keys(body).filter(
        key => rules.required[key] === undefined && rules.optional[key] === undefined
    );
